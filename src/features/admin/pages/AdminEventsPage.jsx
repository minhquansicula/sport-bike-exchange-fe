...
const AdminEventsPage = () => {
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      await eventService.createEvent(newEvent); // Không gửi status
      ...
      toast.error("Lỗi khi tạo sự kiện, vui lòng thử lại.");
    }
  };
  ...
  const handleCancelEvent = async (eventId) => {
    if (window.confirm("Bạn có chắc chắn muốn hủy sự kiện này?")) {
      try {
        await eventService.cancelEvent(eventId); // Cần thêm method trong service
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-5 transition-all hover:shadow-md">
          ...
          <div>
            <p className="text-sm text-slate-500 font-medium mb-1">Đã hủy</p>
            <p className="text-3xl font-black text-slate-900">
